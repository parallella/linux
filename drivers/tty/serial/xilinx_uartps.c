 ...
 * @rxen: Should RX be enabled?
 * @txen: Should TX be enabled?
 * @port: Handle to the uart port structure
 */
static void cdns_uart_rxtx_enable(struct uart_port *port, bool rxen, bool txen)
{
	struct cdns_uart *cdns_uart = port->private_data;
	u32 ctrl_reg;
	...
	ctrl_reg = readl(port->membase + CDNS_UART_CR);
	if (txen) {
		/* Clear the TX disable bit and set the TX enable bit to enable
		 * the transmitter.
		 */
	...
	if (rxen) {
		/* Clear the RX disable bit and set the RX enable bit to enable
		 * the receiver. Unless the CDNS_UART_NO_RX_PULLUP quirk is on
		 * and has triggered, in which case RX is always disabled.
		 */
		...
		unsigned long event, void *data)
{
	...
	case ABORT_RATE_CHANGE:
		...
		/* Enable Tx/Rx */
		cdns_uart_rxtx_enable(port, true, true);
...
static int cdns_uart_startup(struct uart_port *port)
{
	...
	/* Enable Rx */
	cdns_uart_rxtx_enable(port, true, false);
